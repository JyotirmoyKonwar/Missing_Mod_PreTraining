...
class UAHTransformer(nn.Module):
    """
    Transformer-based encoder for the UAH-Driveset.
    """
    def __init__(self, input_dim=9, d_model=128, nhead=4, num_layers=3, num_classes=3):
        """
        Initializes the UAHTransformer model.

        Args:
            input_dim (int): The number of features in the input data (default: 9).
    ...
    def forward(self, x):
        ...
        Args:
            x (torch.Tensor): Input tensor of shape (Batch, Seq_Len, Features), e.g., [B, 64, 9].
        ...
        """
        # x: [B, T=64, 9]
        h = self.embed(x)               # -> [B, 64, 128]
        h = h.permute(1, 0, 2)          # -> [T=64, B, 128] for Transformer
        out = self.transformer(h)       # -> [T=64, B, 128]
